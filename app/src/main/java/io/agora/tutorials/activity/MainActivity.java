...
public class MainActivity extends BasePermissionActivity {
    ...
    private void init() {
        ...
        //获取服务运行状态
        boolean serviceRunning = isServiceRunning(this, MyApplication.getInstance().getServicePackageName());
        if (!serviceRunning) {
            //勿扰模式开着如果服务在运行就关闭
            //关闭服务
        ...
        boolean serviceRunning2 = isServiceRunning(this, MyApplication.getInstance().getServicePackageName());
        if (!serviceRunning2) {
            showMute.setText(R.string.dnd_mode_open);
        } else {
            showMute.setText(R.string.wait_called);
